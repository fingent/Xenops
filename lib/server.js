...
// ORM layer 
var orm = require('orm');
// ORM Database support(MySQL, MariaDB, PostgreSQL, Amazon Redshift, SQLite, MongoDB)
ormDb = null;
// Logger
...
function initApp(){
	...
	if (config.isOrmEnabled) {
		// Format "databasedriver://username:password@host/database"
		var mysqlConStr = 'mysql://root:@localhost/nodedb';
		orm.connect(mysqlConStr, function(err, db){
			if (err) {
